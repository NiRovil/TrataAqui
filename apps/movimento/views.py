...
def upload(request):
    ...
    if request.method == 'POST' and form.is_valid:
        ...
        if len(df) == 0:
            validacao['erro'] = 'O arquivo está em branco!'
        ...
        if data_inicio.date() in data_transacao:
            validacao['erro'] = 'Um arquivo com as mesmas datas e horarios já foi usado para upload!'
            return erro(request, validacao)
        user = request.user
...
def importacoes(request):
    ...
    dados = {'datas':datas}
    return render(request, 'uploads/importacoes.html', dados)
...
def detalhes(request, username):
    if request.method == 'GET':
        ...
        return render(request, 'uploads/detalhes.html', dados)
...
def analise(request):
    ...
    return render(request, 'uploads/analise.html', dados)